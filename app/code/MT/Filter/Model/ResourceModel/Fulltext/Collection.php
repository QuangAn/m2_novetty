 ...
 */
namespace MT\Filter\Model\ResourceModel\Fulltext;
...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Collection
{
    ...
    /**
     * @var \MT\Filter\Model\Request\Builder
     */
     ...
     * @param \MT\Filter\Model\Request\Builder $requestBuilder
    ...
    public function __construct(
        ...
        \MT\Filter\Model\Request\Builder $requestBuilder,
     ...
     * @return $this
     */
    public function addFieldToFilter($field, $condition = null,$key=false)
    {
        if ($this->queryResponse !== null) {
            throw new \RuntimeException('Illegal state');
        }
        if (!is_array($condition) || !in_array(key($condition), ['from', 'to'])) {
            $this->requestBuilder->bind($field, $condition,$key);
        } else {
    ...
    {
        $this->addFieldToFilter('category_ids', $category->getId(),$key);
    ...
    public function getMinMaxValueByAttribute(\Magento\Catalog\Model\ResourceModel\Eav\Attribute $attribute)
    {
        if(!isset($this->_minMaxValuesAttributes[$attribute->getAttributeCode()])) {
            if(
                ...
                $requestBuilder->removeData($attribute->getAttributeCode().".from");
                $requestBuilder->removeData($attribute->getAttributeCode().".to");
    ...
    {
        $requestBuilder = clone $this->_memRequestBuilder;
        $requestBuilder->removeData("price.from");
        $requestBuilder->removeData("price.to");
        $this->requestBuilder = $requestBuilder;
        $this->queryResponse = null;
        $this->_isFiltersRendered = false;
